...
async function sendMailResult(env, to, subject, html) { try { const r=await fetch(EMAIL_WORKER,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'email',secret:env.ONYX_SECRET,to,subject,html,fromLocal:'noreply',fromName:'ONYX CRM'})}); const j=await r.json().catch(()=>({error:`Email worker returned HTTP ${r.status}.`})); return j.ok?{ok:true}:{ok:false,error:j.error||`Email worker returned HTTP ${r.status}.`}; } catch (e) { return {ok:false,error:String(e&&e.message||e)}; } }
async function sendMail(env, to, subject, html) { return (await sendMailResult(env,to,subject,html)).ok; }
...
async function handle(env, b) {
  ...
  if(b.action==='callQueue'){const d=new Date();d.setHours(23,59,59,999);const campaignId=text(b.campaignId),p=[];let sql="SELECT l.* FROM leads l";if(campaignId){sql+=" JOIN campaign_members m ON m.lead_id=l.id AND m.campaign_id=? AND m.status<>'removed'";p.push(campaignId)}sql+=" WHERE l.stage NOT IN ('Won','Lost','Do Not Contact') AND coalesce(l.phone,'')<>''";if(b.mine){sql+=' AND (coalesce(l.owner,\'\')=\'\' OR l.owner=?)';p.push(who.username)}sql+=' AND (coalesce(l.next_action_date,\'\')=\'\' OR l.next_action_date<=?) ORDER BY CASE WHEN coalesce(l.next_action_date,\'\')=\'\' THEN 1 ELSE 0 END,l.lead_score DESC LIMIT ?';p.push(d.toISOString(),Math.min(campaignId?500:100,Number(b.size)||40));const q=(await env.DB.prepare(sql).bind(...p).all()).results.map(x=>summary(leadFrom(x)));return {ok:true,total:q.length,queue:q};}
  if(b.action==='dialerCampaigns'){const rows=(await env.DB.prepare("SELECT c.id,c.name,c.status,count(l.id) callable FROM outreach_campaigns c LEFT JOIN campaign_members m ON m.campaign_id=c.id AND m.status<>'removed' LEFT JOIN leads l ON l.id=m.lead_id AND l.stage NOT IN ('Won','Lost','Do Not Contact') AND coalesce(l.phone,'')<>'' WHERE c.status<>'archived' GROUP BY c.id ORDER BY CASE c.status WHEN 'active' THEN 0 ELSE 1 END,c.created_at DESC").all()).results;return {ok:true,campaigns:rows};}

...
async function handle(env, b) {
  ...
  if(b.action==='logCall'){const r=await getLead(env,b.id);if(!r)return {ok:false,error:'Lead not found.'};r.callAttempts=(Number(r.callAttempts)||0)+1;r.callOutcome=b.outcome||'';r.lastContacted=now();if(b.stage)r.stage=b.stage;if(b.nextAction)r.nextAction=b.nextAction;if(b.nextActionDate)r.nextActionDate=b.nextActionDate;if(!r.owner)r.owner=who.username;r.updatedAt=now();await saveLead(env,r);await log(env,who,r,'call',(b.outcome||'called')+(b.note?' — '+b.note:''));if(b.note){r.notes=(`[${now().slice(0,16).replace('T',' ')} ${who.name}] Call: ${b.note}\n`+(r.notes||'')).slice(0,45000);await saveLead(env,r)}return {ok:true};}
